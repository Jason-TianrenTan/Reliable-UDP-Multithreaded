...
public class UDPServer {
    ...
    FileData fileData;
    byte[] buffer;
    HashMap<String, UDPClientHandler> handlers;
    public UDPServer(int port, FileData fileData) {
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    String signature = clientAddress.toString() + clientPort;
                    if (!handlers.containsKey(signature) || !handlers.get(signature).isActive()) {
                        UDPClientHandler handler = new UDPClientHandler(socket, clientAddress, clientPort, fileData.getFiles(), fileData.getDir(), threadID++);
                        handlers.put(signature, handler);
                        new Thread(handler).start();
                    }
                    handlers.get(signature).updatePacket(packet);
                } catch (Exception e) {
                }
            }
        } catch (IOException e) {
